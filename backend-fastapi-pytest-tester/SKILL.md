...
1. **Response format**: `🤝 <BACKEND_TESTER> ...` (mode emoji + role tag)
...
**Confirmation is handled at invocation** - When user invokes `/backend-fastapi-pytest-tester`, the system prompts `🤝 Invoking <BACKEND_TESTER>. (y/n)`. Once confirmed, proceed without additional confirmation.
